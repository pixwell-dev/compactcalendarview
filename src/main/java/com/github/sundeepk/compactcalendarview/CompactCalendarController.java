...
class CompactCalendarController {
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                ...
                eventIndicatorStyle = typedArray.getInt(R.styleable.CompactCalendarView_compactCalendarEventIndicatorStyle, FILL_LARGE_INDICATOR);
        ...
        for (int dayColumn = 0, colDirection = isRtl? 6 : 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (currentCalender.get(Calendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
                    drawSelectedDayCircleIndicator(currentSelectedDayIndicatorStyle, canvas, xPosition, yPosition, currentSelectedDayBackgroundColor);
                ...
                if (day <= 0) {
                    ...
                    Calendar calendar = monthToDrawCalender;
                    calendar.set(Calendar.DAY_OF_MONTH, day);
                    calendar.get(Calendar.DAY_OF_MONTH);

                    List<Event> dayEvents = eventsContainer.getEventsFor(calendar.getTimeInMillis());

                    if(dayEvents.size() > 0 && currentCalender.get(Calendar.DAY_OF_MONTH) != day) {
                        int tempColor = dayPaint.getColor();
                        dayPaint.setColor(Color.WHITE);
                        canvas.drawText(String.valueOf(day), xPosition, yPosition, dayPaint);
                        dayPaint.setColor(tempColor);
                        dayPaint.setTypeface(Typeface.DEFAULT);
                    } else {
                        canvas.drawText(String.valueOf(day), xPosition, yPosition, dayPaint);
                        dayPaint.setTypeface(Typeface.DEFAULT);
    ...
    private void drawSelectedDayCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color) {
        ...
        drawSelectedCircle(canvas, x, y, Color.rgb(0, 0, 0), 1);
        dayPaint.setStrokeWidth(strokeWidth);
        dayPaint.setStyle(Paint.Style.FILL);
    }

    private void drawSelectedCircle(Canvas canvas, float x, float y, int color, float circleScale) {
        if (animationStatus == ANIMATE_INDICATORS) {
            float maxRadius = circleScale * bigCircleIndicatorRadius * 1.4f;
            dayPaint.setShader(new LinearGradient(x, y, y + (growfactorIndicator > maxRadius ? maxRadius: growfactorIndicator), x + (growfactorIndicator > maxRadius ? maxRadius: growfactorIndicator), Color.rgb(68, 192, 222), Color.rgb(50, 42, 162), Shader.TileMode.CLAMP));
            drawSelectedCircle(canvas, growfactorIndicator > maxRadius ? maxRadius: growfactorIndicator, x, y - (textHeight / 6));
        } else {
            Shader shader = new LinearGradient(x, y - (textHeight / 6), x + (circleScale * bigCircleIndicatorRadius), (y - (textHeight / 6)) + (circleScale * bigCircleIndicatorRadius), Color.rgb(68, 192, 222), Color.rgb(50, 42, 162), Shader.TileMode.CLAMP);
            dayPaint.setShader(shader);
            drawSelectedCircle(canvas, circleScale * bigCircleIndicatorRadius, x, y - (textHeight / 6));
        }
        dayPaint.setShader(null);
    }
    ...
    private void drawEventIndicatorCircle(Canvas canvas, float x, float y, int color) {
        ...
        drawDayWithWhiteCircleIndicator(FILL_LARGE_INDICATOR, canvas, x, y, color);
        //}
    }

    private void drawDayWithWhiteCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color) {
        drawDayWithWhiteCircleIndicator(indicatorStyle, canvas, x, y, color, 1);
    }


    private void drawDayWithWhiteCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color, float circleScale) {
        dayPaint.setColor(Color.WHITE);
        float strokeWidth = dayPaint.getStrokeWidth();
        if (indicatorStyle == NO_FILL_LARGE_INDICATOR) {
            dayPaint.setStrokeWidth(2 * screenDensity);
    ...
    private void drawSelectedCircle(Canvas canvas, float radius, float x, float y) {
